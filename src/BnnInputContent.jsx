...
class BnnInputContent extends React.Component {
    ...
    render(){
        ...
        const archievedItems = this.state.items.filter(item => 
            item.archieved && (
                item.title.toLowerCase().includes(this.state.find.toLowerCase())
            )
        );
            ...
            <>
                <div id="inputContent">
                    <div id="inputContent1">
                        <h1>CARI CATATAN</h1>
                        <input type='text' placeholder='cari catatan...' value={this.state.find} onChange={this.onFindEventHandler}/>
                    </div>
                    <div id="inputContent2">
                        <h1>DAFTAR CATATAN</h1>
                        <form onSubmit={this.onSubmitEventHandler}>
                            <h3>Judul:</h3>
                            <input id="judul" type='text' placeholder='judul catatan' value={this.state.title} onChange={this.onTitleEventHandler}/>
                            <p id="karakter">sisa karakter: {this.state.countChar}</p>
                            <h3>Isi:</h3>
                            <textarea id="isi" type='text' placeholder='isi catatan...' value={this.state.body} onChange={this.onBodyEventHandler}/>
                            <button type="submit">Buat</button>
                        </form>
                    </div>
                </div>
                <BnnActiveContent items={activeItems} onDelete={this.onDeleteHandler} onArchive={this.onArchiveHandler} onActive={this.onActiveHandler}/>
                <BnnArchieveContent items={archievedItems} onDelete={this.onDeleteHandler} onArchive={this.onArchiveHandler} onActive={this.onActiveHandler}/>
            </>
        )
    }
}
